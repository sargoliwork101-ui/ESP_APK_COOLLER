...
// نکته برای توسعه‌دهنده/هوش مصنوعی آینده:
// webpage.h و کل منطق تولید HTML (تابع قدیمی handleRoot) از فرم‌ور حذف شده‌اند.
// ظاهر و منطق پنل اکنون به‌صورت یک اپ اندروید (WebView/Capacitor) مستقل اجرا می‌شود
// و فقط از طریق همین ۹ مسیر JSON/فرم زیر با برد صحبت می‌کند. هیچ‌کدام از این مسیرها
// یا منطق داخلی‌شان تغییر نکرده‌اند؛ فقط یک مسیر جدید GET /config و هدرهای CORS اضافه شده‌اند.
...
// نکته: توابع قدیمی ارسال تکه‌ای HTML (sendHtmlSafe/sendHtmlChunk) که فقط برای
// تولید صفحه‌ی HTML روی خود برد استفاده می‌شدند، چون دیگر برد HTML سرو نمی‌کند حذف شدند
// (کاهش مصرف فلش/رم — همان هدف اصلی این تغییر).
...
void setup() {
  ...
  // مسیر "/" دیگر HTML نمی‌فرستد؛ ظاهر برنامه اکنون داخل اپ اندروید است.
  // به‌جایش یک مسیر سبک GET /config اضافه شده که تنها یک‌بار در شروع اپ خوانده می‌شود
  // و همان مقادیری را برمی‌گرداند که قبلاً داخل خودِ HTML جاسازی می‌شدند (سناریوها، SSID/رمزها و...).
  ...
  // چون حالا درخواست‌ها از یک اپ/WebView با «مبدأ» متفاوت (نه از خود برد) ارسال می‌شوند،
  // مرورگر برای برخی درخواست‌ها (مثلاً بدنه JSON در /save) ابتدا یک درخواست OPTIONS
  // (preflight) می‌فرستد. برای همه‌ی مسیرها یک پاسخ خالی با هدر CORS ثبت می‌کنیم.
  server.on("/config", HTTP_OPTIONS, handleCorsPreflight);
  server.on("/save", HTTP_OPTIONS, handleCorsPreflight);
  server.on("/sync", HTTP_OPTIONS, handleCorsPreflight);
  server.on("/status", HTTP_OPTIONS, handleCorsPreflight);
  server.on("/toggle-manual", HTTP_OPTIONS, handleCorsPreflight);
  server.on("/save-ap", HTTP_OPTIONS, handleCorsPreflight);
  server.on("/save-sta", HTTP_OPTIONS, handleCorsPreflight);
  server.on("/save-protection", HTTP_OPTIONS, handleCorsPreflight);
  server.on("/save-ap-cycle", HTTP_OPTIONS, handleCorsPreflight);
...
// اجازه‌ی دسترسی از مبدأ اپ اندروید/WebView به این وب‌سرور (CORS).
// چون اینجا هیچ کوکی/اطلاعات ورودی حساسی رد و بدل نمی‌شود، "*" (همه مبداها) مشکلی ندارد.
void addCorsHeaders() {
...
// پاسخ به درخواست‌های OPTIONS (preflight) که مرورگر/WebView پیش از برخی درخواست‌های
// cross-origin (مثلاً بدنه JSON) خودکار می‌فرستد.
void handleCorsPreflight() {
  addCorsHeaders();
  server.send(204);
}

// جایگزین سبک تابع قدیمی handleRoot: فقط داده‌های اولیه‌ای که قبلاً داخل خودِ HTML
// جاسازی می‌شدند (سناریوها، SSID/رمز AP و STA و...) را یک‌بار به‌صورت JSON برمی‌گرداند.
// اپ اندروید این مسیر را فقط یک‌بار موقع باز شدن صفحه صدا می‌زند؛ وضعیت زنده همچنان از /status می‌آید.
// منطق و مقداردهی هیچ‌کدام از فیلدها نسبت به نسخه‌ی قبلی HTML تغییری نکرده است.
void handleGetConfig() {
  addCorsHeaders();
  DynamicJsonDocument doc(4096);
  doc["programName"] = PROGRAM_NAME;
  doc["programTagline"] = PROGRAM_TAGLINE;
  doc["manualOverride"] = manual_override;
  doc["apSsid"] = custom_ssid;
  doc["apPass"] = custom_password;
  ...
  doc["apOffMinutes"] = apOffMinutes;
  doc["apTxPowerLevel"] = apTxPowerLevel;
  doc["antiShortCycleMinutes"] = antiShortCycleMinutes;

  JsonArray arr = doc.createNestedArray("scenarios");
  for (int i = 0; i < MAX_SCENARIOS; i++) {
    JsonObject o = arr.createNestedObject();
    o["active"] = scenarios[i].active ? 1 : 0;
    o["enabled"] = scenarios[i].enabled ? 1 : 0;
    o["sh"] = scenarios[i].startHour;
    o["sm"] = scenarios[i].startMinute;
    o["eh"] = scenarios[i].endHour;
    o["em"] = scenarios[i].endMinute;
